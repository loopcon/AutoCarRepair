    ...
    <div class="container-fluid p-0">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="@if(isset($record->id)){{ route('admin_carmaker-update', array('id' => Crypt::encrypt($record->id))) }}@else{{route('admin_carmaker-store')}}@endif" id="page-form" enctype="multipart/form-data" data-parsley-validate="">
                            <input type="hidden" name="id" value="{{ isset($record->id) ? Crypt::encrypt($record->id) : '' }}">
                            {{ csrf_field() }}
                            <div class="form-row">
                                <div class="mb-3 col-md-6">
                                    <label class="form-label" for="title">{{__('Car Maker Title')}}<span class="text-danger">*</span></label>
                                ...
                                <?php /**<div class="mb-3 col-md-12">
                                    ...
                                    <div class="profile-icon">
                                        ...
                                        @if(isset($record->image))
                                            @if($record->image !='')
                                                @php($required = '')
                                                <img style="width:15%;" class='img-responsive previewImage img-fluid' id="uploadPreview{{$i}}" src="{{url('public/uploads/carmaker/'.$record->image)}}"  alt=''>
                                            @else
                            ...
                            <a href="{{route('admin_carmakers')}}" class="btn btn-danger">{{__('Cancel')}}</a>
                        </form>
                    </div>
                </div>
            </div>
