...
<form method="POST" action="@if(isset($record->id)){{ route('admin_car-brand-update', array('id' => Crypt::encrypt($record->id))) }}@else{{route('admin_car-brand-store')}}@endif" id="page-form" enctype="multipart/form-data" data-parsley-validate="">
    {{ csrf_field() }}
